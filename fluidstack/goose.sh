...
run_server "${FLUIDSTACK_SERVER_IP}" "CONFIGURE=false curl -fsSL https://github.com/block/goose/releases/latest/download/download_cli.sh | bash"

# 6. Verify installation succeeded
if ! run_server "${FLUIDSTACK_SERVER_IP}" "command -v goose &> /dev/null && goose --version &> /dev/null"; then
...
sleep 1
clear
interactive_session "${FLUIDSTACK_SERVER_IP}" "source ~/.zshrc && goose"
