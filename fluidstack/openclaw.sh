...
log_info "OpenClaw on FluidStack"
echo ""
...
run_server "${FLUIDSTACK_SERVER_IP}" "/.bun/bin/bun install -g openclaw"
...
run_server "${FLUIDSTACK_SERVER_IP}" "nohup openclaw gateway > /tmp/openclaw-gateway.log 2>&1 &"
sleep 2
interactive_session "${FLUIDSTACK_SERVER_IP}" "source ~/.zshrc && openclaw tui"
