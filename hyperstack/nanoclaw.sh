...
log_info "NanoClaw on Hyperstack"
echo ""
...
# 6. Install Node.js deps and clone nanoclaw
log_warn "Installing tsx..."
run_server "${HYPERSTACK_VM_IP}" "source ~/.bashrc && bun install -g tsx"

log_warn "Cloning and building nanoclaw..."
run_server "${HYPERSTACK_VM_IP}" "git clone https://github.com/gavrielc/nanoclaw.git ~/nanoclaw && cd ~/nanoclaw && npm install && npm run build"
...
upload_file "${HYPERSTACK_VM_IP}" "${DOTENV_TEMP}" "/root/nanoclaw/.env"
...
echo ""
interactive_session "${HYPERSTACK_VM_IP}" "cd ~/nanoclaw && source ~/.zshrc && npm run dev"
