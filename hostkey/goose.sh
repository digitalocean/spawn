...
run_server "${HOSTKEY_INSTANCE_IP}" "CONFIGURE=false curl -fsSL https://github.com/block/goose/releases/latest/download/download_cli.sh | bash"

# Verify installation succeeded
if ! run_server "${HOSTKEY_INSTANCE_IP}" "command -v goose &> /dev/null && goose --version &> /dev/null"; then
    log_install_failed "Goose" "CONFIGURE=false curl -fsSL https://github.com/block/goose/releases/latest/download/download_cli.sh | bash" "${HOSTKEY_INSTANCE_IP}"
    exit 1
fi
...
sleep 1
clear
interactive_session "${HOSTKEY_INSTANCE_IP}" "source ~/.zshrc && goose"
