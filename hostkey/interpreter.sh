...
log_info "Open Interpreter on HOSTKEY"
echo ""
...
run_server "${HOSTKEY_INSTANCE_IP}" "pip install open-interpreter 2>/dev/null || pip3 install open-interpreter"

# Verify installation succeeded
if ! run_server "${HOSTKEY_INSTANCE_IP}" "command -v interpreter &> /dev/null"; then
    log_install_failed "Open Interpreter" "pip install open-interpreter" "${HOSTKEY_INSTANCE_IP}"
    exit 1
fi
...
sleep 1
clear
interactive_session "${HOSTKEY_INSTANCE_IP}" "source ~/.zshrc && interpreter"
