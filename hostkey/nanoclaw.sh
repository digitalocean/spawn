...
run_server "${HOSTKEY_INSTANCE_IP}" "npm install -g tsx"

# 6. Clone nanoclaw
log_step "Cloning nanoclaw..."
run_server "${HOSTKEY_INSTANCE_IP}" "git clone https://github.com/gavrielc/nanoclaw.git ~/nanoclaw && cd ~/nanoclaw && npm install && npm run build"
...
sleep 1
clear
interactive_session "${HOSTKEY_INSTANCE_IP}" "cd ~/nanoclaw && source ~/.zshrc && npm run dev"
