...
run_server "${RUNPOD_POD_ID}" "source ~/.bashrc && bun install -g openclaw"
...
run_server "${RUNPOD_POD_ID}" "source ~/.zshrc && nohup openclaw gateway > /tmp/openclaw-gateway.log 2>&1 &"
sleep 2
interactive_session "${RUNPOD_POD_ID}" "source ~/.zshrc && openclaw tui"
