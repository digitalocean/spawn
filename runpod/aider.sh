...
run_server "${RUNPOD_POD_ID}" "pip install aider-chat 2>/dev/null || pip3 install aider-chat"

if ! run_server "${RUNPOD_POD_ID}" "command -v aider &> /dev/null && aider --version &> /dev/null"; then
...
sleep 1
clear
interactive_session "${RUNPOD_POD_ID}" "source ~/.zshrc && aider --model openrouter/${MODEL_ID}"
