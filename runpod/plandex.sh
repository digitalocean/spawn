...
run_server "${RUNPOD_POD_ID}" "curl -sL https://plandex.ai/install.sh | bash"

if ! run_server "${RUNPOD_POD_ID}" "command -v plandex &> /dev/null && plandex version &> /dev/null"; then
...
sleep 1
clear
interactive_session "${RUNPOD_POD_ID}" "source ~/.zshrc && plandex"
