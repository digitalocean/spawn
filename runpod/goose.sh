...
run_server "${RUNPOD_POD_ID}" "CONFIGURE=false curl -fsSL https://github.com/block/goose/releases/latest/download/download_cli.sh | bash"

if ! run_server "${RUNPOD_POD_ID}" "command -v goose &> /dev/null && goose --version &> /dev/null"; then
...
sleep 1
clear
interactive_session "${RUNPOD_POD_ID}" "source ~/.zshrc && goose"
