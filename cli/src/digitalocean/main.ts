...
async function main() {
  ...
  logInfo(`${agent.name} on DigitalOcean`);
  ...
  const apiKey = await getOrPromptApiKey(agentName, "digitalocean");
  ...
  process.stderr.write("\n");
  logInfo("DigitalOcean droplet setup completed successfully!");
  process.stderr.write("\n");
