// daytona/agents.ts — Agent configs + shared install/config helpers for Daytona
...
/**
 * Upload a config file to the remote machine via base64 SSH channel.
 */
async function uploadConfigFile(
  content: string,
  remotePath: string,
): Promise<void> {
  // Security: validate remotePath to prevent command injection and path traversal
...
export const agents: Record<string, AgentConfig> = {
  ...
  openclaw: {
    ...
    install: () =>
      installAgent(
        "openclaw",
        'source ~/.bashrc && bun install -g openclaw',
      ),
    envVars: (apiKey) => [
      `OPENROUTER_API_KEY=${apiKey}`,
