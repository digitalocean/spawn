...
async function main() {
  ...
  logInfo(`${agent.name} on AWS Lightsail`);
  ...
  const apiKey = await getOrPromptApiKey(agentName, "aws");
  ...
  await createInstance(serverName);
  ...
  process.stderr.write("\n");
  logInfo("AWS Lightsail instance setup completed successfully!");
  process.stderr.write("\n");
