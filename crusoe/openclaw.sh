...
log_info "OpenClaw on Crusoe Cloud"
echo ""
...
run_server "${CRUSOE_VM_IP}" "source ~/.bashrc && bun install -g openclaw"
...
run_server "${CRUSOE_VM_IP}" "source ~/.zshrc && nohup openclaw gateway > /tmp/openclaw-gateway.log 2>&1 &"
sleep 2
interactive_session "${CRUSOE_VM_IP}" "source ~/.zshrc && openclaw tui"
